...
def render_pert_tasks(tasks: any) -> tuple[str, list, list]:
    ...
    for data in tasks['activities']:
        ...
        duracion_media_pert = math.ceil((float(data['duration']['optimistic']) + 4*float(data['duration']['most_likely']) + float(data['duration']['pessimistic'])) / 6)
    ...
    for node in G.nodes(data=True):
        ...
        label = nid if duration is None else f"{nid}\n{duration if duration != 0 else ''}"
