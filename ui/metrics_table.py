...
def evm_table(tasks: dict):
    ...
    if "evm_form_data" not in st.session_state:
        st.session_state.evm_form_data = {
            key: {"BCWP": 0.0, "ACWP": 0.0, "tiempo_trabajado": 0} for key in tasks.keys()
        }

    st.header("Earned Value Management (EVM)")
    st.markdown("#### Complete la siguiente tabla con los valores actuales del proyecto:")

    with st.form("evm_form"):
        # Create the table headers
        cols = st.columns([2, 1, 1, 1, 1, 1])
        headers = ["Tarea", "Presupuesto estimado (BAC)", "Tiempo planeados", 
                  "Valor Ganado (BCWP/EV)", "Costo Real (ACWP/AC)", "Tiempo trabajado"]
    ...
    if submitted:
        df = pd.DataFrame([
            {"TAREA": tasks[key]["task"], "BAC": tasks[key]['BAC'], "duration": tasks[key]['duration'], **st.session_state.evm_form_data[key]}
            for key in tasks.keys()
        ])
        ...
        df["EAC Pesimista"] = np.where(condition,
        ...
        totals = {
            ...
            "SPI": df["BCWP"].astype(float).sum() / df["PV"].astype(float).sum() if df["PV"].astype(float).sum() > 0 else 0,
        ...
        df_with_totals = pd.concat([df, pd.DataFrame([display_totals])], ignore_index=True)
